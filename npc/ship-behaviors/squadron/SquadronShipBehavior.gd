...
func _process(_delta : float):
	if squadron == null:
		return
	_parent.movement_target = _compute_next_target_position()
	queue_redraw()
...
func add_to_squadron(squadron_id: String):
	...
	squadron.add_ship(_parent)

func _compute_next_target_position():
	var proximity_vector = Vector2.ZERO
	for other_ship in squadron.ships:
		if other_ship == null or other_ship == _parent:
			continue
		var vector_from_other_ship = _parent.global_position - other_ship.global_position
	...
	alignment_velocity = squadron.alignment_factor * (squadron.velocity - Helpers.get_velocity(_parent))
	cohesion_velocity = squadron.cohesion_factor * (squadron.global_position - _parent.global_position)
	...
	return _parent.global_position + target_velocity

func _draw():
	if _parent.debug_enabled:
		draw_line(Vector2.ZERO, avoidance_velocity.rotated(-_parent.global_rotation), Color.RED, 2,0)
		draw_line(Vector2.ZERO, alignment_velocity.rotated(-_parent.global_rotation), Color.BROWN, 2,0)
		draw_line(Vector2.ZERO, cohesion_velocity.rotated(-_parent.global_rotation), Color.GREEN, 2,0)
		draw_line(Vector2.ZERO, squadron_target_velocity.rotated(-_parent.global_rotation), Color.YELLOW, 2,0)
